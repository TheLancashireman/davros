 ...
 * Bit3 is an extended task that expects to be sent the ev_Flicker event to generate the next
 * LED state. An alarm sends the event every 89 ms.
 * The Bit3 task also generates the keypresses to test the HID keyboard.
*/
/* Object identifiers
*/
DeclareTask(Led);
...
TASK(Bit3)
{
	...
	for (;;)
	{
		...
		else
		{
			...
			if ( timer == 1 )
			{
				send_hid_report(REPORTID_KEYBOARD, message[count]);
				count++;
			...
			{
				timer = RATE;
				send_hid_report(REPORTID_KEYBOARD, '\0');
			}
		}
	}
